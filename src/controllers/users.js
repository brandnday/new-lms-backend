...
import { executeCreateUpdateDeleteQuery, pgPool } from "../services/database";
...
const getUser = async (username) => {
  const res = await pgPool.query(
    `select id, username,role from tr_user where username = $1`,
    [username]
  );
  return res.rows[0];
};
...
export const signup = async (ctx, next) => {
  ...
  // await pgPool.query(
  //   `insert into tr_user(username,password) values('${username}','${hpass}')`
  // );

  const res = await executeCreateUpdateDeleteQuery(
    `insert into tr_user(username,password,
      name,
      role,
      email,
      school_id,
      ...
      grade_id,
      enroll_year,
      status) values($1,$2,$3,$4,$5,$6,$7,$8,$9,'A')`,
    [username, hpass, name, role, email, school_id, nip, grade_id, enroll_year]
  );
  ctx.ok({ test: "asdf", response: res });
};
